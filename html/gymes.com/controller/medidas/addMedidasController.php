...
function agregarMedidas($medidas) {
    ...
    $result = $medidas->crearMedidas($id_sesion, $torax, $axilas, $busto, $brazo_der, $brazo_izq, $abd_alto, $abd_bajo, $cintura, $cadera, $gluteos, $muslo_der, $muslo_izq, $rodilla_der, $rodilla_izq);

    if ($result) {
        echo json_encode(["status" => "success", "message" => "Medidas agregadas exitosamente."]);
    } else {
        echo json_encode(["status" => "error", "message" => "No se pudo agregar medidas."]);
    }
}

agregarMedidas($medidas);
?>
